# -*- coding: utf-8 -*-
"""
Created on Tues Feb 25 14:26:06 2026

Standard python implementation of IJSEMwebscraper1.7_debug.ipynb Jupyter
notebook implementation includes spacy AI detection for strains, basionyms and organism names.
...
def find_basionyms(description: str):
    """Return only 'basionym' entities from the trained model (no gating).

    Returns a de-duplicated list of basionym strings.
    """
    if not description:
        return []
    results = []
...
def debug_ner(description: str, *, url: str = "", header: str = ""):
    """Minimal debug output for trained NER model (safe for .py)."""
    if not DEBUG_NER:
        return
    ...
    for element in driver.find_elements(By.CSS_SELECTOR, "div.tl-main-part.title"):  # finds section headers
        ...
        if "Description of" in description:
            ...
            for element in driver.find_elements(By.ID, snumber):
                ...
                row_data = [orgname, accessions, strains, basionym_for_row, authority, doi, filtered_url]
                length = len(pub_df)
...
# Ensure we're working on a real copy (prevents SettingWithCopyWarning)
pub_df = pub_df.copy()

# Convert Strains lists -> Excel-safe strings
pub_df.loc[:, 'Strains'] = pub_df['Strains'].apply(
    lambda l: ', '.join(map(str, l)) if isinstance(l, list) and len(l) > 0 else ''
)
pub_df.loc[:, 'Strains'] = pub_df['Strains'].astype(pd.StringDtype())
pub_df.loc[:, 'Strains'] = pub_df['Strains'].str.replace(',', ', ')

# Convert Basionym -> blank when empty (Excel-safe)
pub_df.loc[:, 'Basionym'] = pub_df['Basionym'].apply(
    lambda x: ', '.join(map(str, x)) if isinstance(x, list) and len(x) > 0 else (x if isinstance(x, str) else '')
...
combine_df = combine_df[['PublishedName', 'NCBIname', 'Strains', 'accession', 'strain', 'Basionym',
                         'Authority', 'taxid', 'DOI', 'filtered_url']]
