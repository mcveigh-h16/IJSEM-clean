...
def filter_insdc_accessions(values):
    ...
    for v in values:
        ...
        # Normalize WGS accessions to the WGS master accession.
        # Examples:
        #   JBTKUV010000000 -> JBTKUV000000000
        #   JBTK01000000    -> JBTK00000000
        m = re.match(r"^([A-Z]{4})(\d{8})$", s, re.I)
        if m:
            s = m.group(1) + ("0" * len(m.group(2)))
        else:
            m = re.match(r"^([A-Z]{6})(\d{9})$", s, re.I)
            if m:
                s = m.group(1) + ("0" * len(m.group(2)))
